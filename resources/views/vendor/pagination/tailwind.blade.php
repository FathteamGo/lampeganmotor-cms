        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            ...
            <div>
                <span class="relative z-0 inline-flex rtl:flex-row-reverse shadow-sm rounded-md">
                    ...
                    {{-- Custom Pagination Elements - Show only 3 pages at a time --}}
                    @php
                        $currentPage = $paginator->currentPage();
                        $lastPage = $paginator->lastPage();
                        
                        // Calculate the group of 3 pages
                        $groupStart = ((ceil($currentPage / 3) - 1) * 3) + 1;
                        $groupEnd = min($groupStart + 2, $lastPage);
                        
                        // Show previous group indicator
                        $showPrevGroup = $groupStart > 1;
                        $showNextGroup = $groupEnd < $lastPage;
                    @endphp

                    {{-- Previous Group Link --}}
                    @if ($showPrevGroup)
                        <a href="{{ $paginator->url($groupStart - 1) }}" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-700 bg-white border border-gray-300 leading-5 hover:text-gray-500 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-400 dark:hover:text-gray-300 dark:active:bg-gray-700 dark:focus:border-blue-800">
                            ...
                        </a>
                    @endif
